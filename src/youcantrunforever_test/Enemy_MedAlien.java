...
public class Enemy_MedAlien extends Enemy
{
    final int BASE_DAMAGE = 10; //10-39 damage, 5 range, 10 speed, 55 HP
    final int RAND_DAMAGE = 30;
    final int ATTACK_DISTANCE = 5;
    
    public Enemy_MedAlien(int startingDistanceFromWall)
    {
        super(55, 10, "Warrior", startingDistanceFromWall);
    }
    ...
    public @Override void Attack(Player p)
    {
        ...
        {
            int damage;
            System.out.println(name + " strikes "+p.getName()+" with its claws!");
            if (p.isDefending())
